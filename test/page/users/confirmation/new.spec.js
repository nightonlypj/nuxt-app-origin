...
describe('new.vue', () => {
  ...
  const viewTest = (wrapper, loggedIn, data) => {
    ...
    if (data == null) {
      ...
      helper.mockCalledTest(routerPushMock, 1, { path: '/users/confirmation/new' }) // Tips: URLパラメータを消す為
    }
  ...
  const apiCalledTest = (values) => {
    ...
    expect(axiosPostMock).nthCalledWith(1, helper.envConfig.apiBaseURL + helper.commonConfig.confirmationNewUrl, {
      email: values.email,
