...
describe('[code].vue', () => {
  ...
  it('[ログイン中][管理者、削除予定なし]表示される', async () => {
    ...
    // 確認ダイアログ
    const dialog: any = wrapper.find('#space_delete_dialog')
    expect(dialog.exists()).toBe(true)
    expect(dialog.isVisible()).toBe(true) // 表示

    // はいボタン
    const yesButton: any = wrapper.find('#space_delete_yes_btn')
    expect(yesButton.exists()).toBe(true)
    expect(yesButton.element.disabled).toBe(false) // 有効

    // いいえボタン
    const noButton: any = wrapper.find('#space_delete_no_btn')
