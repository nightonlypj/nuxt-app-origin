...
import { defaultParams, findParams, findQuery, findData, dataCount0, dataCount1, dataCount2, dataPage1, dataPage2, dataPage3, dataPageTo2, dataPageTo3, dataPageMiss1, dataPageMiss2 } from '~/test/data/members'
...
describe('[code].vue', () => {
  ...
  const model = 'member'
  const defaultHiddenItems = $config.public.members.defaultHiddenItems
  ...
  const apiCalledTest = (count: number, params: object = { ...defaultParams, page: count }) => {
    ...
    expect(mock.useApiRequest).nthCalledWith(count, apiRequestURL.value(helper.locale, $config.public.members.listUrl.replace(':space_code', space.code)), 'GET', params)
  }
  ...
  const viewTest = (wrapper: any, data: any, countView: string, admin = false, show: any = { existInfinite: false, testState: null }, error = false) => {
    ...
    if (data.members == null || data.members.length === 0) {
      expect(wrapper.text()).toMatch($t('対象の{name}が見つかりません。', { name: $t('メンバー') }))
    ...
    expect(wrapper.text()).toMatch(countView) // [1名以上]人数
  ...
  const infiniteErrorTest = async (messages = {}, signOut = false) => {
    ...
    apiCalledTest(1)
    const infiniteLoading = viewTest(wrapper, dataPage1, $t('{total}名（複数）', { total: 5 }), false, { existInfinite: true, testState: null })
    ...
    if (signOut) {
      ...
      viewTest(wrapper, dataPage1, $t('{total}名（複数）', { total: 5 }), false, { existInfinite: true, testState: 'error' }, true)
    }
  }
  ...
  describe('メンバー一覧取得', () => {
    ...
    describe('1名', () => {
      it('[管理者]表示される（招待・変更・ダウンロード・削除・結果も含む）', async () => {
        ...
        apiCalledTest(1)
        viewTest(wrapper, data, $t('{total}名（単数）', { total: 1 }), true)
      ...
      it('[管理者以外]表示される（招待・変更・ダウンロードを除く）', async () => {
        ...
        apiCalledTest(1)
        viewTest(wrapper, dataCount1, $t('{total}名（単数）', { total: 1 }), false)
      })
    })
    describe('2名', () => {
      it('[管理者]表示される（招待・変更・ダウンロード・削除・結果も含む）', async () => {
        const data = Object.freeze({ ...dataCount2, space: detailPower.value('admin') })
        mock.useApiRequest = vi.fn(() => [{ ok: true, status: 200, headers: mock.headers }, data])
        const wrapper = mountFunction()
        helper.loadingTest(wrapper, AppLoading)
        await flushPromises()
        ...
        apiCalledTest(1)
        viewTest(wrapper, data, $t('{total}名（複数）', { total: 2 }), true)
      ...
      it('[管理者以外]表示される（招待・変更・ダウンロードを除く）', async () => {
        ...
        apiCalledTest(1)
        viewTest(wrapper, dataCount2, $t('{total}名（複数）', { total: 2 }), false)
      })
    })
    describe('無限スクロール', () => {
      ...
      const beforeAction = async (uid1: string | null, uid2: string | null, uid3: string | null = null) => {
        ...
        apiCalledTest(1)
        infiniteLoading = viewTest(wrapper, dataPage1, $t('{total}名（複数）', { total: 5 }), false, { existInfinite: true, testState: null })
      }
      const completeTestAction = async () => {
        ...
        apiCalledTest(2)
        infiniteLoading = viewTest(wrapper, dataPageTo2, $t('{total}名（複数）', { total: 5 }), false, { existInfinite: true, testState: 'loaded' })
        ...
        apiCalledTest(3)
        viewTest(wrapper, dataPageTo3, $t('{total}名（複数）', { total: 5 }), false, { existInfinite: false, testState: 'complete' })
      }
  ...
  describe('メンバー一覧検索', () => {
    ...
    const beforeAction = async (reloading = false) => {
      ...
      apiCalledTest(1)
      viewTest(wrapper, dataPage1, $t('{total}名（複数）', { total: 5 }), false, { existInfinite: true, testState: null })

      // メンバー一覧検索
      wrapper.vm.$refs.membersSearch.updateWaiting = vi.fn()
