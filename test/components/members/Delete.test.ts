...
describe('Delete.vue', () => {
  ...
  describe('メンバー解除', () => {
    const apiCalledTest = () => {
      ...
      const url = helper.commonConfig.members.deleteUrl.replace(':space_code', space.code)
      expect(mock.useApiRequest).nthCalledWith(1, helper.envConfig.apiBaseURL + url, 'POST', { codes })
    }
